...
public class JetpackItem extends Item implements Colored, Enableable {
    ...
    /*
     * Jetpack logic is very much like Simply Jetpacks, since I used it to learn how
     * to make this work
     * Credit to Tonius & Tomson124
     * https://github.com/Tomson124/SimplyJetpacks-2/blob/1.12/src/main/java/tonius/
     * simplyjetpacks/item/rewrite/ItemJetpack.java
     */
    @Override
    public void inventoryTick(ItemStack stack, net.minecraft.server.level.ServerLevel worldIn,
            net.minecraft.world.entity.Entity entityIn, @org.jspecify.annotations.Nullable EquipmentSlot slot) {
        if (!(entityIn instanceof Player player) || slot != EquipmentSlot.CHEST)
            return;
    ...
    public void tickJetpack(ItemStack stack, Player player) {
            ...
            if (jetpack.isEngineOn(stack)) {
                ...
                if (InputHandler.isHoldingUp(player) || hover && !player.onGround()) {
                    ...
                    if (player.getItemBySlot(EquipmentSlot.CHEST) == stack) {
                        energy = EnergyStorage.ITEM.find(stack,
                                net.fabricmc.fabric.api.transfer.v1.context.ContainerItemContext
                                        .ofSingleSlot(new com.blakebr0.ironjetpacks.item.storage.ItemSlotStorage(player,
                                                EquipmentSlot.CHEST)));
                    }
                    if (energy == null && net.fabricmc.loader.api.FabricLoader.getInstance().isModLoaded("trinkets")) {
                        var access = com.blakebr0.ironjetpacks.compat.trinkets.TrinketsCompat
                                .getEquippedJetpackAccess(player);
                        if (access != null && access.get() == stack) {
                            energy = EnergyStorage.ITEM.find(stack,
                                    net.fabricmc.fabric.api.transfer.v1.context.ContainerItemContext.ofSingleSlot(
                                            new com.blakebr0.ironjetpacks.compat.trinkets.TrinketSlotStorage(access)));
                        }
                    }

                    try (Transaction transaction = Transaction.openOuter()) {
                        long extracted = creative ? (long) usage
                                : (energy != null ? energy.extract((long) usage, transaction) : 0);
                        if (creative || extracted > 0) {
                            ...
                            if (!player.level().isClientSide()) {
                                if (player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) {
                                    ((com.blakebr0.ironjetpacks.mixins.ServerPlayNetworkHandlerAccessor) serverPlayer.connection)
                                            .setFloatingTicks(0);
                                }
                            }
                        }
        ...
        if (!player.level().isClientSide()) {
            chargeJetpack(stack, player);
        }
    }

    public void chargeJetpack(ItemStack jetpack, Player player) {
        if (player.level().isClientSide())
            return;

        EnergyStorage jetpackEnergy = null;
        if (player.getItemBySlot(EquipmentSlot.CHEST) == jetpack) {
            jetpackEnergy = EnergyStorage.ITEM.find(jetpack,
                    net.fabricmc.fabric.api.transfer.v1.context.ContainerItemContext.ofSingleSlot(
                            new com.blakebr0.ironjetpacks.item.storage.ItemSlotStorage(player, EquipmentSlot.CHEST)));
        } else if (net.fabricmc.loader.api.FabricLoader.getInstance().isModLoaded("trinkets")) {
            var access = com.blakebr0.ironjetpacks.compat.trinkets.TrinketsCompat.getEquippedJetpackAccess(player);
            if (access != null && access.get() == jetpack) {
                jetpackEnergy = EnergyStorage.ITEM.find(jetpack,
                        net.fabricmc.fabric.api.transfer.v1.context.ContainerItemContext.ofSingleSlot(
                                new com.blakebr0.ironjetpacks.compat.trinkets.TrinketSlotStorage(access)));
            }
        ...
        if (jetpackEnergy == null || jetpackEnergy.getCapacity() - jetpackEnergy.getAmount() <= 0)
            return;

        boolean isTrinketsLoaded = net.fabricmc.loader.api.FabricLoader.getInstance().isModLoaded("trinkets");
        boolean isBackpacksLoaded = net.fabricmc.loader.api.FabricLoader.getInstance().isModLoaded("tiered_backpacks");

        if (isTrinketsLoaded) {
            var component = eu.pb4.trinkets.api.TrinketsApi.getAttachment(player);
            if (component != null) {
                var equipped = component.getEquipped(s -> true);
                for (var tuple : equipped) {
                    var ref = tuple.getA();
                    if (ref.reference().slot().endsWith("back")) {
                        ItemStack backStack = tuple.getB();
                        if (!backStack.isEmpty() && backStack != jetpack) {
                            EnergyStorage batteryEnergy = EnergyStorage.ITEM.find(backStack,
                                    ContainerItemContext.ofSingleSlot(
                                            new com.blakebr0.ironjetpacks.compat.trinkets.TrinketSlotStorage(
                                                    ref)));
                            if (batteryEnergy != null && batteryEnergy.supportsExtraction()
                                    && batteryEnergy.getAmount() > 0) {
                                try (net.fabricmc.fabric.api.transfer.v1.transaction.Transaction tx = net.fabricmc.fabric.api.transfer.v1.transaction.Transaction
                                        .openOuter()) {
                                    long jetpackCapacity = jetpackEnergy.getCapacity() - jetpackEnergy.getAmount();
                                    long extracted = batteryEnergy.extract(jetpackCapacity, tx);
                                    if (extracted > 0) {
                                        long inserted = jetpackEnergy.insert(extracted, tx);
                                        if (inserted != extracted) {
                                            tx.abort();
                                            try (net.fabricmc.fabric.api.transfer.v1.transaction.Transaction tx2 = net.fabricmc.fabric.api.transfer.v1.transaction.Transaction
                                                    .openOuter()) {
                                                long ex2 = batteryEnergy.extract(inserted, tx2);
                            ...
                            if (isBackpacksLoaded) {
                                com.blakebr0.ironjetpacks.compat.backpacks.TieredBackpacksCompat
                                        .tryChargeFromBackpack(jetpack, jetpackEnergy, player, backStack);
                            }
                        }
                    }
                }
        ...
        if (isBackpacksLoaded) {
            for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
                ...
                if (!invStack.isEmpty() && invStack != jetpack) {
                    com.blakebr0.ironjetpacks.compat.backpacks.TieredBackpacksCompat.tryChargeFromBackpack(jetpack,
                            jetpackEnergy, player, invStack);
                }
            }
        }
    }
    ...
    @Override
    public void appendHoverText(final ItemStack stack, final Item.TooltipContext context, final TooltipDisplay display,
            final java.util.function.Consumer<Component> builder, final TooltipFlag tooltipFlag) {
        if (!this.jetpack.creative) {
            EnergyStorage energy = EnergyStorage.ITEM.find(stack, ContainerItemContext.withConstant(stack));
            builder.accept(
                    Component.literal(UnitUtils.formatEnergy(energy.getAmount(), null)).withStyle(ChatFormatting.GRAY)
                            .append(" / ").append(Component.literal(UnitUtils.formatEnergy(jetpack.capacity, null))));
        } else {
            builder.accept(Component.literal("-1 E / ").withStyle(ChatFormatting.GRAY)
                    .append(ModTooltips.INFINITE.color(ChatFormatting.GRAY)).append(" E"));
        }

        Component tier = ModTooltips.TIER.args(this.jetpack.creative ? "Creative" : this.jetpack.tier)
                .withStyle(this.jetpack.rarity.color());
        ...
        if (ModConfigs.getClient().general.enableAdvancedInfoTooltips) {
            ...
            if (!com.mojang.blaze3d.platform.InputConstants.isKeyDown(Minecraft.getInstance().getWindow(),
                    org.lwjgl.glfw.GLFW.GLFW_KEY_LEFT_SHIFT)
                    && !com.mojang.blaze3d.platform.InputConstants.isKeyDown(Minecraft.getInstance().getWindow(),
    ...
    public boolean isEngineOn(ItemStack stack) {
        return stack
                ...
                .copyTag().getBoolean("Engine").orElse(false);
    }
    ...
    public boolean isHovering(ItemStack stack) {
        return stack
                .getOrDefault(net.minecraft.core.component.DataComponents.CUSTOM_DATA,
                        net.minecraft.world.item.component.CustomData.EMPTY)
                .copyTag().getBoolean("Hover").orElse(false);
    }
