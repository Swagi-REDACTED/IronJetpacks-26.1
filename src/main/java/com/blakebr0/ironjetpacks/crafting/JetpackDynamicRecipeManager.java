...
public class JetpackDynamicRecipeManager {
    public static void appendRecipes(BiConsumer<Identifier, Recipe<?>> appender) {
        JetpackRegistry.getInstance().getAllJetpacks().forEach(jetpack -> {
            makeCellRecipe(jetpack, appender);
            makeThrusterRecipe(jetpack, appender);
            makeCapacitorRecipe(jetpack, appender);
            makeJetpackRecipe(jetpack, appender);
            makeJetpackUpgradeRecipe(jetpack, appender);
        });
    }
    
    private static void makeCellRecipe(Jetpack jetpack, BiConsumer<Identifier, Recipe<?>> appender) {
    ...
    private static void makeThrusterRecipe(Jetpack jetpack, BiConsumer<Identifier, Recipe<?>> appender) {
    ...
    private static void makeCapacitorRecipe(Jetpack jetpack, BiConsumer<Identifier, Recipe<?>> appender) {
    ...
    private static void makeJetpackRecipe(Jetpack jetpack, BiConsumer<Identifier, Recipe<?>> appender) {
    ...
    private static void makeJetpackUpgradeRecipe(Jetpack jetpack, BiConsumer<Identifier, Recipe<?>> appender) {
        if (!ModConfigs.get().recipe.enableJetpackRecipes)
            return;
        ...
        Identifier name = Identifier.fromNamespaceAndPath(IronJetpacks.MOD_ID, jetpack.name + "_jetpack");
        JetpackUpgradeRecipe recipe = new JetpackUpgradeRecipe(new Recipe.CommonInfo(true), new CraftingRecipe.CraftingBookInfo(CraftingBookCategory.EQUIPMENT, ""), pattern, new ItemStackTemplate(jetpack.item.get()));
        appender.accept(name, recipe);
    }
}
